...
**5. The failure is repairable, and the repair identifies the cause.** Rewriting TrpB's serine into the aminoacrylate its cofactor forms — one reaction SMARTS, no model, no training, no new data — moves Morgan difference from 0.37× the pool's composition to **3.28×** and DRFP to **97.7% (3.83×)**, while carbene figures stay unchanged because no rule applies to them. Seeds returning no same-family candidate drop from 20/12/2/19 to 1/1/0/8. No normalised seed matches a pool entry exactly, so this is resemblance at the intermediate level rather than leakage. RXNFP is the one representation that does not recover, reaching only 0.77×. Two explanations for that were offered and both **refuted**: with both sides fully reduced to the free core the three substructure representations reach ×3.95–×4.92 and leave no seed without a same-family candidate, while RXNFP stays at ×0.78. Measuring the space instead of the query explains it — the dehydroamino acid family has a separation of **+0.015** in RXNFP's space against **+0.352** for carbene, so it is not a region there at all. Normalisation aligns a query to a family; it cannot create a neighbourhood a representation does not encode. See the [cross-family report](outputs/candidate_pool/CROSS_FAMILY.md).

**6. Catalytic context reorders the shortlist, by an amount that depends on the family.** Labelling candidates as "known transferable" does not survive the data — at a 0.3 threshold the dehydroamino acid pool yields **3** positives — so context is reported as **obstacles**: what a procedure would have to change to run in a protein. For PLP, structural ranking already returns 0.1 obstacles against a pool average of 0.9, and context adds almost nothing. For carbene, structural ranking returns 1.9 against a pool average of 2.0, and ordering by obstacles surfaces a **disjoint** top-10 led by **hemin catalysing carbene transfer in water at 20 °C** plus three metal-free carbene reactions. See the [transfer-obstacle report](outputs/candidate_pool/TRANSFER_OBSTACLES.md).
...
The suite contains 168 tests. 167 run in CI on Python 3.11 and 3.12 from the checkout alone; the remaining one verifies RXNFP's 256-dimensional CLS output against the official README example within 1e-5 and needs locally downloaded weights. CI separately regenerates the technical note from `summary.json` and fails if the committed note differs, so no reported metric can be hand-edited. See the report for the evidence chain and limitations.
