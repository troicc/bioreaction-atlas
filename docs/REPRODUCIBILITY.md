...
1. **Tests** on Python 3.11 and 3.12. The suite contains 105 tests; the RXNFP integration test is deselected because it requires locally downloaded official weights, so 104 run in CI. A green run therefore demonstrates that the tests pass on a clean machine, not only in the original workspace.
